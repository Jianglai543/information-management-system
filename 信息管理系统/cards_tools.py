...
def show_all():
    ...
    """
    print("显示全部名片: ")
    print("共有%d人信息" % len(list))
    for x in list:
        print("名字：%s 电话：%s qq: %s 电子邮件：%s" % (x["name"], x["phone"], x["qq"], x["email"]))

# 查询
def check_menu():
    print("查询名片: ")
    namex = input("输入你想查什么（名字或电话）： ")
    for y in list:
        if namex in y["name"] or namex in y["phone"]:
            print("名字：%s 电话：%s qq: %s 电子邮件：%s" % (y["name"], y["phone"],y["qq"],y["email"]))#y 是一字典
